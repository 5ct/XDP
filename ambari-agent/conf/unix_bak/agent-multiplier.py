...
class Multiplier:
  ...
  """
  CONFIG_FILE = "/etc/xdp-agent/conf/agent-multiplier.conf"
  ...
  def __init__(self, args):
    ...
    self.log_dir = "/var/log/xdp-agent"
    self.config_dir = "/etc/xdp-agent/conf"
    self.pid_file = "/var/run/xdp-agent/xdp-agent.pid"
    self.prefix_dir = "/var/lib/xdp-agent/data"
    self.cache_dir = "/var/lib/xdp-agent/cache"

    # Ambari Agent config file to use as a template
    # Will change hostname and port after copying
    self.source_config_file = "/etc/xdp-agent/conf/xdp-agent.ini"
    self.source_version_file = "/var/lib/xdp-agent/data/version"
  ...
  def validate(self):
    ...
    if not os.path.isfile(self.source_config_file):
      errors.append("xdp Agent config file does not exist at %s" % self.source_config_file)
  ...
  def bootstrap(self):
    ...
    for host in self.hosts:
      ...
      # Copy config file
      host_config_file = os.path.join(host_config_dir, "xdp-agent.ini")
  ...
  def cmd_start(self):
    ...
    for host in self.hosts:
      cmd = "xdp-agent start --home %s" % (host.home_dir)
      os.environ['AMBARI_AGENT_CONF_DIR'] = os.path.join(host.home_dir, "etc/xdp-agent/conf")
      subprocess.call(cmd, shell=True, env=os.environ)

  def cmd_stop(self):
    print "Stopping %d host(s)" % len(self.hosts)
    for host in self.hosts:
      cmd = "xdp-agent stop --home %s" % (host.home_dir)
      os.environ['AMBARI_AGENT_CONF_DIR'] = os.path.join(host.home_dir, "etc/xdp-agent/conf")
      subprocess.call(cmd, shell=True, env=os.environ)
  ...
  def cmd_restart(self):
    ...
    for host in self.hosts:
      cmd = "xdp-agent restart --home %s" % (host.home_dir)
      os.environ['AMBARI_AGENT_CONF_DIR'] = os.path.join(host.home_dir, "etc/xdp-agent/conf")
      subprocess.call(cmd, shell=True, env=os.environ)
