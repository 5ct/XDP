...
INSTALL_HELPER_SERVER="/var/lib/xdp-server/install-helper.sh"
COMMON_DIR_AGENT="/usr/lib/xdp-agent/lib/ambari_commons"
RESOURCE_MANAGEMENT_DIR_AGENT="/usr/lib/xdp-agent/lib/resource_management"
JINJA_AGENT_DIR="/usr/lib/xdp-agent/lib/ambari_jinja2"
SIMPLEJSON_AGENT_DIR="/usr/lib/xdp-agent/lib/ambari_simplejson"
AMBARI_AGENT="/usr/lib/python2.6/site-packages/ambari_agent"
PYTHON_WRAPER_TARGET="/usr/bin/ambari-python-wrap"
AMBARI_AGENT_VAR="/var/lib/xdp-agent"
...
do_install(){
  if [ -d "/etc/xdp-agent/conf.save" ]; then
    cp -f /etc/xdp-agent/conf.save/* /etc/xdp-agent/conf
    mv /etc/xdp-agent/conf.save /etc/xdp-agent/conf_$(date '+%d_%m_%y_%H_%M').save
  fi
  ...
  if [ "$?" -eq 0 ] ; then
    chkconfig --add xdp-agent
  fi
  which update-rc.d > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    update-rc.d xdp-agent defaults
  fi
  ...
  BAK=/etc/xdp-agent/conf/ambari-agent.ini.old
  ORIG=/etc/xdp-agent/conf/ambari-agent.ini
  UPGRADE_AGENT_CONFIGS_SCRIPT=/var/lib/xdp-agent/upgrade_agent_configs.py

  if [ -z "$AMBARI_PYTHON" ] ; then
    >&2 echo "Cannot detect python for xdp to use. Please manually set $PYTHON_WRAPER_TARGET link to point to correct python binary"
    >&2 echo "Cannot upgrade agent configs because python for xdp is not configured. The old config file is saved as $BAK . Execution of $UPGRADE_AGENT_CONFIGS_SCRIPT was skipped."
  else
...
do_remove(){
  /usr/sbin/xdp-agent stop > /dev/null 2>&1
  ...
  mv /etc/xdp-agent/conf /etc/xdp-agent/conf.save
  ...
  which chkconfig > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    chkconfig --list | grep xdp-server && chkconfig --del xdp-agent
  fi
  which update-rc.d > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    update-rc.d -f xdp-agent remove
  fi
}
