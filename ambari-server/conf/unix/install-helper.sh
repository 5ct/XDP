...
AMBARI_SERVER_EXECUTABLE_LINK="${ROOT}/usr/sbin/xdp-server"
AMBARI_SERVER_EXECUTABLE="${ROOT}/etc/init.d/xdp-server"
...
do_install(){
  ...
  if [ "$?" -eq 0 ] ; then
    AUTOSTART_SERVER_CMD="chkconfig --add xdp-server"
  fi
  which update-rc.d > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    AUTOSTART_SERVER_CMD="update-rc.d xdp-server defaults"
  fi
...
do_remove(){
  ...
  which chkconfig > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    chkconfig --list | grep xdp-server && chkconfig --del xdp-server
  fi
  which update-rc.d > /dev/null 2>&1
  if [ "$?" -eq 0 ] ; then
    update-rc.d -f xdp-server remove
  fi
}
