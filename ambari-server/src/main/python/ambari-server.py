...
#
# Starts the xdp Server as a standalone process.
...
def start(options):
  ...
  if status:
    err = "xdp Server is already running."
...
#
# Starts the xdp Server.
# Ensures only one instance of the process is running.
#     If this is the second instance of the process, the function fails.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def start(args):
  logger.info("Starting xdp-server.")
  status, pid = is_server_runing()
  if status:
    err = "xdp Server is already running."
    raise FatalException(1, err)
  ...
  logger.info("Started xdp-server.")


#
# Starts the xdp Server as a service.
# Start the server as a Windows service. If the xdp server is
...
#
# Stops the xdp Server.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def stop(args):
  logger.info("Stopping xdp-server.")
  if (args != None):
  ...
  if status:
    try:
      ...
      err = "Unable to stop xdp Server - " + str(e)
      print_info_msg(err)
      raise FatalException(1, err)
    ...
    print "xdp Server stopped"
    logger.info("xdp Server stopped")
  else:
    print "xdp Server is not running"
    logger.info("xdp Server is not running")


#
# Restarts the xdp Server.
#
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def restart(args):
  logger.info("Restarting xdp-server.")
  stop(args)
  start(args)
...
@OsFamilyFuncImpl(OsFamilyImpl.DEFAULT)
def status(args):
  logger.info("Get status of xdp-server.")
  args.exit_message = None
  status, pid = is_server_runing()
  pid_file_path = os.path.join(configDefaults.PID_DIR, PID_NAME)
  if status:
    args.exit_code = 0
    print "xdp Server running"
    print "Found xdp Server PID: " + str(pid) + " at: " + pid_file_path
  else:
    print "xdp Server not running. Stale PID File at: " + pid_file_path
    args.exit_code = 3
...
def create_setup_security_actions(args):
  action_list = [
      ['setup-https', 'Enable HTTPS for xdp server.', UserActionRestart(setup_https, args)],
      ['encrypt-passwords', 'Encrypt passwords stored in xdp.properties file.', UserAction(setup_master_key, args)],
      ['setup-kerberos-jaas', 'Setup xdp kerberos JAAS configuration.', UserAction(setup_ambari_krb5_jaas, args)],
...
def init_parser_options(parser):
  ...
  parser.add_option('-g', '--debug', action="store_true", dest='debug', default=False,
                    help="Start xdp-server in debug mode")
  parser.add_option('-y', '--suspend-start', action="store_true", dest='suspend_start', default=False,
                    help="Freeze xdp-server Java process at startup in debug mode")
...
def init_parser_options(parser):
  ...
  parser.add_option('--existing', action="store_true", default=False,
                    help="LDAP sync existing option.  Synchronize existing xdp users and groups only.", dest="ldap_sync_existing")
  ...
  parser.add_option('--api-ssl', default=None, help="Enable SSL for xdp API [true/false]", dest="api_ssl")
  ...
  parser.add_option('--jaas-principal', default=None, help="Kerberos principal for xdp server", dest="jaas_principal")
...
def are_cmd_line_db_args_valid(options):
  if (options.database_host is not None and options.database_host is not "" \
      #and options.database_name is not None \         # xdp by default is ok
...
def main(options, args, parser):
  ...
  options.exit_message = "xdp Server '%s' completed successfully." % action
  ...
  try:
    if action in _action_option_dependence_map:
      ...
      for opt_str, opt_dest in required:
        if hasattr(options, opt_dest) and getattr(options, opt_dest) is None:
          ...
          print "Run xdp-server.py --help to see detailed description of each option"
    ...
    if options.warnings:
      for warning in options.warnings:
        ...
        pass
      options.exit_message = "xdp Server '%s' completed with warnings." % action
      pass
...
def mainBody():
  ...
  if options.verbose:
    ...
    try:
      ...
      "\nFor more info run xdp-server with -v or --verbose option")
      sys.exit(1)     
