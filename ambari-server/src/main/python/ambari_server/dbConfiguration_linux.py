...
class LinuxDBMSConfig(DBMSConfig):
  ...
  def _reset_remote_database(self):
    ...
    print_warning_msg('To reset XDP Server schema ' +
  ...
  def _setup_remote_db(self):
    setup_msg = "Before starting XDP Server, you must run the following DDL " \
...
class PGConfig(LinuxDBMSConfig):
  ...
  def __init__(self, options, properties, storage_type):
    ...
    self.JDBC_DRIVER_INSTALL_MSG = 'Before starting XDP Server, ' \
                                   'you must copy the {0} JDBC driver JAR file to {1} and set property "server.jdbc.driver.path=[path/to/custom_jdbc_driver]" in XDP.properties.'.format(
  ...
  def _change_tables_owner(self):
    ...
    for query in get_tables_queries:
      ...
      if retcode != 0 or self._check_for_psql_error(stdout, stderr):
        print_error_msg("Failed to get list of XDP tables. Message from psql:\n"
                        " stdout:{0}\n"
    ...
    if not tables:
      print_error_msg("Failed to get list of XDP tables")
      return False
...
class SQLAConfig(LinuxDBMSConfig):
  ...
  def __init__(self, options, properties, storage_type):
    ...
    self.JDBC_DRIVER_INSTALL_MSG = 'Before starting XDP Server, ' \
                                   'you must copy the {0} jdbc client tarball to {1} and set property "server.jdbc.driver.path=[path/to/custom_jdbc_driver]" in XDP.properties.'.format(
