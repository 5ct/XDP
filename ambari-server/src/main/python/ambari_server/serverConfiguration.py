...
class ServerConfigDefaults(object):
  def __init__(self):
    ...
    self.SERVER_OUT_FILE = os.path.join(self.OUT_DIR, "xdp-server.out")
    self.SERVER_LOG_FILE = os.path.join(self.OUT_DIR, "xdp-server.log")
    self.DB_CHECK_LOG = os.path.join(self.OUT_DIR, "xdp-server-check-database.log")
...
class ServerConfigDefaultsWindows(ServerConfigDefaults):
  def __init__(self):
    ...
    #Standard messages
    self.MESSAGE_SERVER_RUNNING_AS_ROOT = "xdp Server running with 'root' privileges."
    self.MESSAGE_WARN_SETUP_NOT_ROOT = "xdp-server setup is run with root-level privileges, passwordless sudo access for some commands commands may be required"
    self.MESSAGE_ERROR_RESET_NOT_ROOT = "xdp-server reset must be run with administrator-level privileges"
    self.MESSAGE_ERROR_UPGRADE_NOT_ROOT = "xdp-server upgrade must be run with administrator-level privileges"
...
class ServerConfigDefaultsLinux(ServerConfigDefaults):
  def __init__(self):
    ...
    #Standard messages
    self.MESSAGE_SERVER_RUNNING_AS_ROOT = "xdp Server running with administrator privileges."
    self.MESSAGE_WARN_SETUP_NOT_ROOT = "xdp-server setup is run with root-level privileges, passwordless sudo access for some commands commands may be required"
    self.MESSAGE_ERROR_RESET_NOT_ROOT = "xdp-server reset should be run with root-level privileges"
    self.MESSAGE_ERROR_UPGRADE_NOT_ROOT = "xdp-server upgrade must be run with root-level privileges"
    self.MESSAGE_CHECK_FIREWALL = "Checking firewall status..."
...
def get_ambari_version(properties):
  ...
  try:
    ...
    print_error_msg("Error getting xdp version")
  return version
...
def update_database_name_property(upgrade=False):
  try:
    ...
    try:
      ...
      err = 'Could not write xdp config file "%s": %s' % (conf_file, e)
      raise FatalException(-1, err)
