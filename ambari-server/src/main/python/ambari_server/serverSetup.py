...
def verify_setup_allowed(options):
  if get_silent():
    ...
    if properties == -1:
      print_error_msg("Error getting xdp properties")
      return -1
    ...
    if isSecure:
      ...
      if not isPersisted:
        ...
        print "xdp Server 'setup' exiting."
        return 1
...
class xdpUserChecksWindows(xdpUserChecks):
  def __init__(self, options):
    super(xdpUserChecksWindows, self).__init__()

    self.NR_USER_CHANGE_PROMPT = "xdp-server service is configured to run under user '{0}'. Change this setting [y/n] ({1})? "
    self.NR_USER_CUSTOMIZE_PROMPT = "Customize user account for xdp-server service [y/n] ({0})? "
  ...
  def _create_custom_user(self):
    user = get_validated_string_input(
      "Enter user account for xdp-server service ({0}):".format(self.user),
      self.user, None,
      "Invalid username.",
      False
    )
    ...
    print_warning_msg("When using non SYSTEM user make sure that your user has read\write access to log directories and "
                      ...
                      "user is properly configured to access the xdp database.")
...
class xdpUserChecksLinux(xdpUserChecks):
  def __init__(self, options):
    super(xdpUserChecksLinux, self).__init__()

    self.NR_USER_CHANGE_PROMPT = "xdp-server daemon is configured to run under user '{0}'. Change this setting [y/n] ({1})? "
    self.NR_USER_CUSTOMIZE_PROMPT = "Customize user account for xdp-server daemon [y/n] ({0})? "
  ...
  def _create_custom_user(self):
    user = get_validated_string_input(
      "Enter user account for xdp-server daemon ({0}):".format(self.user),
      self.user,
      "^[a-z_][a-z0-9_-]{1,31}$",
      "Invalid username.",
      False
...
def check_firewall():
  ...
  if firewall_on:
    print_warning_msg("%s is running. Confirm the necessary xdp ports are accessible. " %
                      firewall_obj.FIREWALL_SERVICE_NAME +
                      "Refer to the xdp documentation for more details on ports.")
    ok = get_YN_input("OK to continue [y/n] (y)? ", True)
    if not ok:
      raise FatalException(1, None)
  ...
  def download_and_install_jdk(self, args, properties):
    ...
    if self.jdk_index == self.custom_jdk_number:
      ...
      if not os.path.exists(args.java_home) or not os.path.isfile(os.path.join(args.java_home, "bin", self.JAVA_BIN)):
        ...
        raise FatalException(1, err)
      print "Validating JDK on xdp Server...done."
    ...
    if os.path.exists(dest_file):
      ...
      ok = get_YN_input("To download the Oracle JDK and the Java Cryptography Extension (JCE) "
                        ...
                        "cancel the xdp Server setup and you must install the JDK and JCE "
                        "files manually.\nDo you accept the "
      ...
      if file_exists:
        ...
        if not ok:
          err = "Unable to install JDK. Please remove JDK file found at " + \
                dest_file + " and re-run xdp Server setup"
          raise FatalException(1, err)
        else:
          jdk_url = jdk_cfg.url
        ...
        err = "Unable to install JDK. File " + dest_file + " does not exist, " \
                                                           "please re-run xdp Server setup"
        raise FatalException(1, err)
...
def download_and_install_jdk(options):
  properties = get_ambari_properties()
  if properties == -1:
    err = "Error getting xdp properties"
    raise FatalException(-1, err)
...
#
# Configures the OS settings in xdp properties.
#
...
def _setup_database(options):
  properties = get_ambari_properties()
  if properties == -1:
    raise FatalException(-1, "Error getting xdp properties")

  factory = DBMSConfigFactory()

  dbmsxdp = factory.create(options, properties, "xdp")
  resultA = dbmsxdp.configure_database(properties, options)

  # Now save the properties file
  if resultA:
    update_properties(properties)

    dbmsxdp.setup_database()
...
def _createDefDbFactory(options):
  ...
  if not (properties.getPropertyDict().has_key(JDBC_URL_PROPERTY) and
    ...
    raise FatalException(-1, "xdp Server not set up yet. Nothing to reset.")
...
def _reset_database(options):
  ...
  if persistence_type == "remote":
      err = 'xdp doesn\'t support resetting exernal DB automatically. ' \
            'To reset xdp Server schema you must first drop and then create it ' \
            'using DDL scripts from "{0}"'.format(AmbariPath.get("/var/lib/ambari-server/resources/"))
      raise FatalException(1, err)
  else:
    factory = DBMSConfigFactory()

    dbmsxdp = factory.create(options, properties)
    dbmsxdp.reset_database()
...
#
# Setup the xdp Server.
#
def setup(options):
  logger.info("Setup xdp-server.")
  if options.only_silent:
    if check_setup_already_done():
      print "Nothing was done. xdp Setup already performed and cannot re-run setup in silent mode. Use \"xdp-server setup\" command without -s option to change xdp setup."
      sys.exit(0)
...
#
# Setup the JCE policy for xdp Server.
#
def setup_jce_policy(args):
  logger.info("Setup JCE policy for xdp-server.")
  ...
  if not jdk_path or not os.path.exists(jdk_path):
    err = "JDK not installed, you need to run 'xdp-server setup' before attempting to install the JCE policy."
    raise FatalException(1, err)
  ...
  print 'NOTE: Restart xdp Server to apply changes' + \
        ' ("xdp-server restart|stop|start")'


#
# Resets the xdp Server.
#
def reset(options):
  logger.info("Reset xdp-server.")
  if not is_root():
  ...
  if status:
    err = 'xdp-server must be stopped to reset'
    raise FatalException(1, err)
