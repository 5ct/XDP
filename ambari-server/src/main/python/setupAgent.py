...
def installAgent(projectVersion, ret=None):
  ...
  if OSCheck.is_suse_family():
    Command = ["zypper", "--no-gpg-checks", "install", "-y", "xdp-agent-" + projectVersion]
    ...
    Command = ["yum", "-y", "install", "--nogpgcheck", "xdp-agent-" + projectVersion]
...
def runAgent(passPhrase, expected_hostname, user_run_as, verbose, ret=None):
  ...
  cmd = ['su', user_run_as, '-l', '-c', '/usr/sbin/xdp-agent restart --expected-hostname=%1s %2s' % (expected_hostname, vo)]
  log = ""
...
def findNearestAgentPackageVersion(projectVersion):
  ...
  if OSCheck.is_suse_family():
    ...
    Command = ["bash", "-c", "yum -q list all xdp-agent | grep '" + projectVersion +
...
def run_setup(argv=None):
  ...
  logmessage = str(retcode["exitstatus"])+'--------------'
